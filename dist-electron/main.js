...
const getConfigPath = () => path.join(app.getPath("userData"), `${TUNNEL_NAME}.conf`);
function getWireGuardCommands(configPath) {
  if (process.platform === "darwin") {
    ...
    const PATH_PREFIX = fs.existsSync("/opt/homebrew/bin") ? "/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin" : "/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin";
    return {
...
function runSudoCommand(command) {
  return new Promise((resolve, reject) => {
    ...
    sudoPrompt.exec(command, sudoOptions, (error, stdout) => {
      if (error) {
...
function isAlreadyExistsError(err) {
  ...
  return msg.includes("already exists") || msg.includes("already in use") || msg.includes("already exists as");
}
async function handleConnect(event, configContent) {
  try {
